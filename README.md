...
| Scaling or normalization | Similar to *Affine* but $Q$ only for scaling or normalization, thus very sparse. | $\mathcal{O}(m)$ | $\mathcal{O}(m^2)$ | With *Affine*, small preconditioner_max_size or preconditioner_max_skew reduces large $Q$ to diagonal; no class for 'normalization' now. | 
...
A few more details. The Hessian-vector products are calculated as ${\rm autograd.grad}(g, \theta, v)$, maybe not the most efficient way for a specific problem, e.g., see [this](https://iclr-blogposts.github.io/2024/blog/bench-hvp/). For gradient whitening preconditioner, preconditioner update and gradient preconditioning share some computations but the current implementations do not exploit this for the sake of readability (no sharing for Newton type preconditioner). Except for the Affine preconditioners, no native support of complex parameter optimizations yet (you can define complex parameters as view of real ones for such problems). No line search is implemented for the conventional convex optimization setting.     
