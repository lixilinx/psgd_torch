...
| Scaling or normalization | Similar to *Affine*, but $Q_1$ or $Q_2$ only for scaling or normalization. | $\mathcal{O}(m)$ | $\mathcal{O}(m^2)$ | With *Affine*, small preconditioner_max_size or preconditioner_max_skew reduces large $Q$ to diagonal. | 
