...
The [new PSGD implementation](https://github.com/lixilinx/psgd_torch/blob/master/psgd.py) is a superset of the [old one](https://github.com/lixilinx/psgd_torch/blob/master/preconditioned_stochastic_gradient_descent.py) (deprecated), and further supports four more matmul-only/inverse-free geometries for updating $Q$. The choices $dQ=Q^{0.5} \mathcal{E} Q^{1.5}$ (default) and $dP=P^{0.5} \mathcal{E} P$ recover the Newton-Schulz (NS) iterations with stability guarantee. A few wrapping examples are provided:
* [Simplest](https://github.com/lixilinx/psgd_torch/blob/master/simplest_standalone_psgd.py) (<=2D tensors) and [simple](https://github.com/lixilinx/psgd_torch/blob/master/simple_standalone_psgd.py) (any order tensors) standalone DDP wrappings. These are the simple dependency-free PSGD momentum whitening optimizers using Kron preconditioner fitted with $dQ=Q^{0.5} \mathcal{E} Q^{1.5}$ and update rule $Q\leftarrow Q - \mu (Pgg^TP - I)Q$, which recover the NS iterations for the inverse 4th root of $E[gg^T]$. **They are standard, light, fast and the recommended implementations**.  
* [DDP wrapping](https://github.com/lixilinx/psgd_torch/blob/master/wrapped_as_torch_optimizer_for_ddp.py) and [FSDP wrapping](https://github.com/lixilinx/psgd_torch/blob/master/wrapped_as_torch_optimizer_for_dtensor.py). They are wrapping examples of the functions inside [psgd.py](https://github.com/lixilinx/psgd_torch/blob/master/psgd.py) for gradient/momentum whitening of tensors of any order (with noticeable overhead cost due to einsum!). For most problems, whitening either gradient or momentum works equally well. For problems with sparse gradients, typically momentum whitening is preferred.
* My customized non-standard wrapping inside [psgd.py](https://github.com/lixilinx/psgd_torch/blob/master/psgd.py). It supports a wide range of choices: gradient/momentum whitening, Hessian fitting with Hvp, different geometries and Lie groups for $Q$ ...          
...
| $Q={\rm diag}(q)$ | $q\leftarrow \left( 1 - \mu \frac{(q\cdot h)^2 - (v\cdot/q)^2}{  \max\left((q\cdot h)^2 + (v\cdot/q)^2\right)} \right) \cdot  q$ | $\mathcal{O}(m^2)$ | $\mathcal{O}(m^2)$ | LRAWhiten/Newton  |
...
#### Implementation details for [psgd.py](https://github.com/lixilinx/psgd_torch/blob/master/psgd.py) 
...
3) As any other regularizations, (coupled) weight decay should be explicitly realized by adding an $L2$ regularization to the loss. Similarly, decoupled weight decay is not included inside these PSGD class implementations. <!--We recommend to randomize the regularization term, e.g., replacing the $L2$ one for a parameter $p$, say $0.5 \lambda \cdot {\rm sum}(p^2)$, with ${\rm rand}() \cdot \lambda\cdot {\rm sum}(p^2)$.--> 
