...
for epoch in range(10):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        Hv = grad(grads, Ws, v)#error? check torch bug: https://github.com/pytorch/pytorch/issues/15532   
        with torch.no_grad():
...
plt.subplot(2,1,1)
plt.semilogy(range(1,11), TrainLoss, '-k', linewidth=0.2)
plt.xlabel('Epochs')
plt.ylabel('Train cross entropy loss')
plt.subplot(2,1,2)
plt.semilogy(range(1,11), TestLoss, '-k', linewidth=0.2)
plt.xlabel('Epochs')
