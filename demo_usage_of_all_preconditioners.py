...
"""
Gradient descent as a base line
"""
xyz = copy.deepcopy(xyz0)
[w.requires_grad_(True) for w in xyz]
opt = torch.optim.SGD(xyz, lr=0.0001) # diverges easily with lr=0.0005; doesn't make progress with lr=0.0002
f_values = []
t0 = time.time()
for epoch in range(num_iterations):
    opt.zero_grad()
    f_value = f(*xyz) + 2**(-23) * sum([torch.sum(torch.rand_like(p) * p * p) for p in xyz])
...
"""
LBFGS as one more base line
"""
xyz = copy.deepcopy(xyz0)
[w.requires_grad_(True) for w in xyz]
opt = torch.optim.LBFGS(xyz, lr=0.1) # diverges easily with lr=0.5; diverges occasionally with lr=0.2
f_values = []
t0 = time.time()
for epoch in range(num_iterations):
...
xyz = copy.deepcopy(xyz0)
[w.requires_grad_(True) for w in xyz]
opt = psgd.XMat(xyz, preconditioner_init_scale=None, lr_params=0.2, lr_preconditioner=0.1)
...
for _ in range(num_iterations):
    ...
    def closure():
        return f(*xyz) + 2**(-23) * sum([torch.sum(torch.rand_like(p) * p * p) for p in opt._params_with_grad])
...
xyz = copy.deepcopy(xyz0)
[w.requires_grad_(True) for w in xyz]
opt = psgd.Newton(xyz, preconditioner_init_scale=None, lr_params=0.5, lr_preconditioner=0.2)
...
xyz = copy.deepcopy(xyz0)
[w.requires_grad_(True) for w in xyz]
opt = psgd.LRA(xyz, preconditioner_init_scale=None, lr_params=0.2, lr_preconditioner=0.1)
...
ax1.legend(["Gradient descent", "LM-BFGS", "PSGD-Xmat", "PSGD-Newton", "PSGD-LRA", "PSGD-Affine"], fontsize=8)
ax1.set_title("Tensor rank decomposition benchmark", loc="left")

ax2.set_xlabel("Wall time (s)")
ax2.set_ylabel("Fitting loss")
ax2.legend(["Gradient descent", "LM-BFGS", "PSGD-Xmat", "PSGD-Newton", "PSGD-LRA", "PSGD-Affine"], fontsize=8)
