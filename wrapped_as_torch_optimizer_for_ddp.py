...
# we can have some settings to force GPU matmul determinstic here, but it could slow down the code. 
...
class WhitenMomentumNS4(torch.optim.Optimizer):
    ...
    def __init__(
            ...
            lr_params=3e-4, # roughly sqrt((1+momentum)/(1-momentum)) times smaller than Adam's lr  
            lr_preconditioner=0.1, 
            betaL=0.9, 
            damping=1e-9, # roughly the eps=1e-8 in Adam 
            momentum=0.9, # momentum is not always helpful; reduce it if so
        ...
        if self.is_distributed: # if True, assume distributed GPU training; important to sync the rng states
    ...
    def step(self):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # resync states occasionally if matmul is not determinstic; 
                # no resync if matmul is determinstic or all preconditioners are diagonal    
                if self.is_distributed and (state["step"] % 10000 == 0):
