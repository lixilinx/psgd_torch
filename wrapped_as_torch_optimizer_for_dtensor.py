...
class WhitenMomentumNS4(torch.optim.Optimizer):
    ...
    This wrapping preconditions each slices of a DTensor gradient independently, not an optimal but generally acceptable solution.  
    """
    ...
    def __init__(
            ...
            preconditioner_max_skew=8.0, # 0.0 => all diagonal Q; inf => all dense Q
            ...
            resync_every=1000_000_000, # resync every # steps if states divergence too much due to nondeterminstic matmul; NOT implemented and generally no need   
    ):
        defaults = {
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if preconditioner_dtype:
                    grad = grad.to_local().squeeze().to(preconditioner_dtype) # squeeze out dim=1; also good to merge small dims if possible
                else:
                    grad = grad.to_local().squeeze()
                ...
                if update_preconditioner and grad.numel() > 0: # no need to update preconditioner for empty sharded slices 
                ...
                local_p = p.to_local()
                local_p.subtract_(group["lr_params"]/torch.clamp(amp/group["grad_clip_max_amp"], min=1.0) * h.view_as(local_p))
                ...
                if state["step"] % group["resync_every"] == 0:
                    pass # check p.device_mesh and p.placements for groups with duplicated states and resync them as DDP wrapping if needed.     
...
if __name__ == "__main__":
    ...
    print(f"""rank {rank} got local preconditioner states {opt.state[model.w]["QL"]}""") 